...
export const updateIngredientStatusById = ({ ingredientId, jwt }) => async (dispatch) => {
    dispatch({ type: UPDATE_INGREDIENT_STATUS_REQUEST })
    try {
        const { data } = await api.get(`/api/admin/ingredient/status/${ingredientId}`, {
            headers: {
                Authorization: `Bearer ${jwt}`
            }
        })
        dispatch({ type: UPDATE_INGREDIENT_STATUS_SUCCESS, payload: data })
    } catch (error) {
        dispatch({ type: UPDATE_INGREDIENT_STATUS_FAILURE, payload: error })
    }
}